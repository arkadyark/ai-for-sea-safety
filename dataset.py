...
class SafetyDataset(Dataset):
    def __init__(self, labels_file, features_path, num_files=10):
        ...
        for column in data.columns:
            if column == 'bookingID': continue
            normalized_data[column] = (data[column] - data[column].mean())/(data[column].max() - data[column].min())
        grouped_data = normalized_data.sort_values('second').groupby('bookingID')
        ...
        self.data = torch.zeros(len(grouped_data), minutes, seconds, INPUT_DIM)
        ...
        for i in range(len(data_groups)):
            ...
            for minute in range(min(int(len(sequence)/seconds), minutes)):
                sequence_minute = sequence[minute*seconds:(minute+1)*seconds, [2, 3, 4, 5, 6, 7, 8, 10]]
